...
(
	function ( $ ) {
		 ...
		 */
		var Libreria_Customizer_FrameWork = {
			...
			checkControlVisibility : function ( control, id ) {
				...
				if ( ! _.isUndefined( control ) ) {
					...
					if ( 'undefined' != typeof LibreriaCustomizerControlsToggle[id] ) {
						var check            = false,
						    dependency_param = LibreriaCustomizerControlsToggle[id],
						...
						if ( 'undefined' !== typeof conditions ) {
							...
							if ( ! check ) {
								control.container.addClass( 'libreria-hide' );
							} else {
								control.container.removeClass( 'libreria-hide' );
							}
						}
					}
			...
			checkDependency : function ( conditions, values, compare_operator ) {
				...
				if ( _.isString( testValue ) ) {
					...
					if ( ! _.isUndefined( LibreriaCustomizerControlsToggle[testValue] ) ) {
						var conditions = ! _.isUndefined( LibreriaCustomizerControlsToggle[testValue]['conditions'] ) ? LibreriaCustomizerControlsToggle[testValue]['conditions'] : LibreriaCustomizerControlsToggle[testValue];
						var operator   = ! _.isUndefined( LibreriaCustomizerControlsToggle[testValue]['operator'] ) ? LibreriaCustomizerControlsToggle[testValue]['operator'] : 'AND';
						...
						if ( ! _.isUndefined( conditions ) ) {
							...
							if ( ! control.checkDependency( conditions, values, operator ) ) {
								...
								control_obj.container.removeClass( 'libreria-hide' );
							}
						}
					}
					...
					$.each(
						...
						function ( index, val ) {
							...
							if ( 'undefined' !== typeof LibreriaCustomizerControlsToggle[cond_key] ) {
								var conditions = ! _.isUndefined( LibreriaCustomizerControlsToggle[cond_key]['conditions'] ) ? LibreriaCustomizerControlsToggle[cond_key]['conditions'] : LibreriaCustomizerControlsToggle[cond_key];
								var operator   = ! _.isUndefined( LibreriaCustomizerControlsToggle[cond_key]['operator'] ) ? LibreriaCustomizerControlsToggle[cond_key]['operator'] : 'AND';
			...
			hideEmptySections : function () {
				...
				$( 'ul.accordion-section.control-section-libreria_section' ).each(
					function () {
						...
						if ( controls.length > 0 ) {
							controls.each(
								function () {
									if ( ! $( this ).hasClass( 'libreria-hide' ) && $( this ).css( 'display' ) != 'none' ) {
										visibleIt = true;
									}
								}
							);
							...
							if ( ! visibleIt ) {
								$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'libreria-hide' );
							} else {
								$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'libreria-hide' );
							}
						}
					}
				);
		...
		$(
			function () {
				Libreria_Customizer_FrameWork.init();
			}
		);
