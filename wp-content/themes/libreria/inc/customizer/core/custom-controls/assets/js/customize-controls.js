...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['libreria-background'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				control.initLibreriaBackgroundControl();

			},

			initLibreriaBackgroundControl : function () {

				var control     = this,
				    value       = control.setting._value,
				    colorpicker = control.container.find( '.libreria-color-picker-alpha' );
				...
				control.container.on( 'click', '.background-image-upload-remove-button', function ( e ) {
					...
					if ( preview.length ) {
						preview.removeClass().addClass( 'placeholder' ).html( LibreriaCustomizerControlBackground.placeholder );
					}
 ...
 * @package Libreria
 */
wp.customize.controlConstructor[ 'libreria-buttonset' ] = wp.customize.Control.extend( {
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor[ 'libreria-color' ] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				var control = this,
					...
					picker = this.container.find( '.libreria-color-picker-alpha' ),
 ...
 * @package Libreria
 */
wp.customize.controlConstructor['libreria-dimensions'] = wp.customize.Control.extend( {
 ...
 * @package Libreria
 */
wp.customize.controlConstructor[ 'libreria-dropdown-categories' ] = wp.customize.Control.extend( {
 ...
 * @package Libreria
 */
wp.customize.controlConstructor[ 'libreria-editor' ] = wp.customize.Control.extend( {
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['libreria-fontawesome'] = wp.customize.Control.extend(
			{
				ready: function () {
					'use strict';
					...
					control.initLibreriaFontawesomeControl();
				},

				initLibreriaFontawesomeControl: function() {
					var control       = this,
						selector      = control.selector,
						elSelector    = $( selector ).find( 'select' ),
						faData        = [],
						value         = control.setting._value,
						data          = window['LibreriaCustomizerControlFontawesome' + this.id],
						faDataCounter = 0,
						faSelect;
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['libreria-group'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				this.container.on( 'libreria_settings_changed', control.onOptionChange );
			...
			registerToggleEvents : function () {
				...
				$( '.wp-full-overlay-sidebar-content' ).click(
					function ( e ) {
						if ( ! $( e.target ).closest( '.libreria-field-settings-modal' ).length ) {
							$( '.libreria-group-toggle-icon.open' ).trigger( 'click' );
						}
					}
				);
				...
				control.container.on(
					'click',
					'.libreria-group-wrap .libreria-group-toggle-icon',
					function ( e ) {
						...
						var $this          = $( this ),
						    parent_wrap    = $this.closest( '.customize-control-libreria-group' ),
						    is_loaded      = parent_wrap.find( '.libreria-field-settings-modal' ).data( 'loaded' ),
						    parent_section = parent_wrap.parents( '.control-section' );

						if ( $this.hasClass( 'open' ) ) {
							parent_wrap.find( '.libreria-field-settings-modal' ).hide();
						} else {

							/* Close popup when another popup is clicked */
							var get_open_popup = parent_section.find( '.libreria-group-toggle-icon.open' );
							if ( get_open_popup.length > 0 ) {
								get_open_popup.trigger( 'click' );
							}

							if ( is_loaded ) {
								parent_wrap.find( '.libreria-field-settings-modal' ).show();
							} else {

								var fields     = control.params.libreria_fields,
								    modal_wrap = $( LibreriaCustomizerControlGroup.group_modal_tmpl ),
								    device     = $( '#customize-footer-actions .active' ).attr( 'data-device' );

								parent_wrap.find( '.libreria-field-settings-wrap' ).append( modal_wrap );
								parent_wrap.find( '.libreria-fields-wrap' ).attr( 'data-control', control.params.name );
								control.libreria_render_field( parent_wrap, fields, control );
								parent_wrap.find( '.libreria-field-settings-modal' ).show();
				...
				control.container.on(
					'click',
					'.libreria-group-wrap > .customizer-text',
					function ( e ) {

						e.preventDefault();
						e.stopPropagation();

						$( this ).find( '.libreria-group-toggle-icon' ).trigger( 'click' );
			...
			libreria_render_field : function ( wrap, fields, control_element ) {

				var control             = this;
				var libreria_field_wrap = wrap.find( '.libreria-fields-wrap' );
				var fields_html         = '';
				var control_types       = [];
				...
				if ( 'undefined' != typeof fields.tabs ) {
					...
					var counter = 0;
					fields_html += '<div id="' + control_element.params.name + '-tabs" class="libreria-group-tabs">';

					fields_html += '<ul class="libreria-group-list">';
					_.each(
						fields.tabs,
						function ( value, key ) {
							var li_class = '';
					...
					fields_html += '<div class="libreria-tab-content" >';
					_.each(
						fields.tabs,
					...
					libreria_field_wrap.html( fields_html );
				...
				_.each(
					...
					function ( control_type, index ) {
						...
						switch ( control_type.key ) {
							...
							case 'libreria-color':
								control.initColorControl( libreria_field_wrap, control_element, control_type.name );
								break;

							case 'libreria-background':
								control.initBackgroundControl( control_element, control_type, control_type.name );
								break;

							case 'libreria-typography':
				...
				wrap.find( '.libreria-field-settings-modal' ).data( 'loaded', true );
			...
			generateFieldHtml : function ( fields_data, field_values ) {
				...
				_.each(
					...
					function ( attr, index ) {
						...
						if ( (
							     'libreria-typography' === attr.control
						     ) && controlsType.includes( attr.control ) ) {
							attr.languages = LibreriaCustomizerControlTypographySubsets;
						}
			...
			initColorControl : function ( wrap, control_elem, name ) {
				...
				var control     = this;
				var colorpicker = wrap.find( '.customize-control-libreria-color .libreria-color-picker-alpha' );
				...
				colorpicker.wpColorPicker(
					{
						change : function ( event, ui ) {
							...
							if ( 'undefined' != typeof event.originalEvent || 'undefined' != typeof ui.color._alpha ) {
								...
								var picker  = $( '#' + name + '.customize-control-libreria-color .libreria-color-picker-alpha' );
			...
			getTypographyFonts : function () {
				...
				if ( ! _.isUndefined( LibreriaCustomizerControlTypography ) ) {
					return LibreriaCustomizerControlTypography;
				}
 ...
 (
	function ( $ ) {
		...
		$( window ).on( 'load', function () {
			...
			$( '.libreria-navigate a' ).on( 'click', function ( e ) {
				e.preventDefault();
 ...
 * @package Libreria
 */
wp.customize.controlConstructor[ 'libreria-radio-image' ] = wp.customize.Control.extend( {
 ...
 * @package Libreria
 */
wp.customize.controlConstructor['libreria-sortable'] = wp.customize.Control.extend( {
 ...
 * @package Libreria
 */
wp.customize.controlConstructor['libreria-slider'] = wp.customize.Control.extend( {
	...
	ready : function () {
		...
		this.container.find( 'input[type=range]' ).on( 'input change', function () {
			var value        = jQuery( this ).val(),
				input_number = jQuery( this ).closest( '.slider-wrapper' ).find( '.libreria-range-value .value' );
		...
		this.container.find( '.libreria-slider-reset' ).click( function () {
			var wrapper       = jQuery( this ).closest( '.slider-wrapper' ),
			    input_range   = wrapper.find( 'input[type=range]' ),
			    input_number  = wrapper.find( '.libreria-range-value .value' ),
 ...
 * @package Libreria
 */
wp.customize.controlConstructor['libreria-toggle'] = wp.customize.Control.extend( {
 ...
 * @package Libreria
 */
wp.customize.controlConstructor['libreria-typography'] = wp.customize.Control.extend( {
